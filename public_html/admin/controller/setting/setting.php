...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		//Watermark
		if (!$this->model_setting_ecos->watermark_check()) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			//We need image size
            if (true || is_file(DIR_IMAGE . $this->request->post['config_watermark_image'])){
                $info = getimagesize(DIR_IMAGE . $this->request->post['config_watermark_image']);
            } else {
                $info = [0,0];
                $this->request->post['config_watermark_image'] = '';
            }
            $this->request->post['config_watermark_size_x'] = $info[0];
            $this->request->post['config_watermark_size_y'] = $info[1];

            //Contacts
            foreach ($this->request->post['config_contacts_data'] as $customer_group_id => $vars) {
            	foreach ($vars['addresses'] as $address_key => $address) {
            		$empty = true;

            		// Clean array from empty addresses
            		foreach ($address as $key => $value) {
            			$s = $this->request->post['config_contacts_data'][$customer_group_id]['addresses'][$address_key][$key]['value'];
            			$s = trim($s);
            			$this->request->post['config_contacts_data'][$customer_group_id]['addresses'][$address_key][$key]['value'] = $s;
            			if (!empty($s)) {
            				$empty = false;
            			}
            		}
            		...
            		if ($empty) {
            			unset($this->request->post['config_contacts_data'][$customer_group_id]['addresses'][$address_key]);
            		}
            	}
            	
            	// Re-index addresses array
            	$this->request->post['config_contacts_data'][$customer_group_id]['addresses'] = array_values($this->request->post['config_contacts_data'][$customer_group_id]['addresses']);
            }
            $this->request->post['config_contacts_data'] = json_encode($this->request->post['config_contacts_data']);
		...
		if (isset($this->request->post['config_schema_address'])) {
			$data['config_schema_address'] = $this->request->post['config_schema_address'];
		} else {
			$data['config_schema_address'] = $this->config->get('config_schema_address');
		}
		...
		if (isset($this->request->post['config_seo_url_short_category_path'])) {
			$data['config_seo_url_short_category_path'] = $this->request->post['config_seo_url_short_category_path'];
		} else {
			$data['config_seo_url_short_category_path'] = $this->config->get('config_seo_url_short_category_path');
		}		
		...
		//Watermark
		$data += $this->model_setting_ecos->watermark_get_setting();
		$data['config_watermark_placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		if ($data['config_watermark_image']) {
			$data['config_watermark_thumb'] = $this->model_tool_image->resize($data['config_watermark_image'], 100, 100);
		} else {
			$data['config_watermark_thumb'] = $data['config_watermark_placeholder'];
		}

		//Contacts
		// $this->load->model('customer/customer_group');
		// $data['customer_groups'] = $this->model_customer_customer_group->getCustomerGroups();
		// $data['customer_group_id'] = isset($this->request->get['customer_group_id']) ? $this->request->get['customer_group_id'] : 0;

		$data['config_contacts_data'] = array();
		$contacts_data = json_decode($this->config->get('config_contacts_data'), true);
		$data_keys = array(
			'email' => array(
				"type" => "text",
				"label" => $this->language->get('entry_contacts_data_email'),
				"value" => "",
			),
			'workhours' => array(
				"type" => "text",
				"label" => $this->language->get('entry_contacts_data_workhours'),
				"value" => "",
			),
			'addresses' => array(
				array(
					"name" => array(
						"type" => "text",
						"label" => $this->language->get('entry_contacts_data_name'),
						"value" => "",
					),
					"address" => array(
						"type" => "textarea",
						"label" => $this->language->get('entry_contacts_data_address'),
						"tooltip" => $this->language->get('entry_contacts_data_address_tooltip'),
						"value" => "",
					),
					"phone" => array(
						"type" => "textarea",
						"label" => $this->language->get('entry_contacts_data_phone'),
						"tooltip" => $this->language->get('entry_contacts_data_phone_tooltip'),
						"value" => "",
					),
					"coords" => array(
						"type" => "text",
						"label" => $this->language->get('entry_contacts_data_coords'),
						"value" => "",
						"tooltip" => $this->language->get('entry_contacts_data_coords_tooltip'),
					),
				),
			)
		);
		...
		if (isset($contacts_data[0])) {
			$data_keys_temp = $this->add_keys('addresses', $data_keys, $contacts_data[0]);
			$data['config_contacts_data'][0] = $contacts_data[0];
			$data['config_contacts_data'][0] = array_replace_recursive($data_keys_temp, $data['config_contacts_data'][0]);
		} else {
			$data['config_contacts_data'][0] = $data_keys;
		}

		$data['contact_customer_groups'] = array();
		foreach ($data['customer_groups'] as $key => $value) {
			$id = $value["customer_group_id"];
			if (isset($contacts_data[$id])) {
				$data_keys_temp = $this->add_keys('addresses', $data_keys, $contacts_data[$id]);
				$data['config_contacts_data'][$id] = $contacts_data[$id];
				$data['config_contacts_data'][$id] = array_replace_recursive($data_keys_temp, $data['config_contacts_data'][$id]);
			} else {
				$data['config_contacts_data'][$id] = $data_keys;
			}
			$data['contact_customer_groups'][$id]["name"] = $value["name"];
		}
	...
	private function add_keys($key, $data_keys, $data) {
		foreach ($data[$key] as $k => $v) {
			if (!isset($data_keys[$key][$k])) {
				$data_keys[$key][$k] = $data_keys[$key][0];
			}
		}
