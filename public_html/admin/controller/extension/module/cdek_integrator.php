...
class ControllerExtensionModuleCdekIntegrator extends Controller {
	...
	public function updateCities() {
		...
		foreach ($countries as $country) {
			...
			for ($page = 0; ; $page++) {
				$url = "https://integration.cdek.ru/v1/location/cities/json?size=5000&page={$page}&countryCode={$country}";  
				$ch = curl_init();
				...
				foreach ($out as $value) {
					if (!isset($check_unique[$value['cityCode']])) {
						$data[] = array(
							'id' => $value['cityCode'],
							'name' => $value['cityName'] . (isset($value['region']) ? (', ' . $value['region']) : NULL),
							'cityName' => $value['cityName'],
							'regionName' => isset($value['region']) ? $value['region'] : $value['country'],
						);
						
						$check_unique[$value['cityCode']] = 1;
					}
				}
