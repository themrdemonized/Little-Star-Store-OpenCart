...
class SeoPro {
		...
		if (!empty($parts) && is_array($parts)) {
			...
			foreach($parts as $id => $part) {
				...
				if($parts[$id]) {
					...
					$query = isset($url[1]) ? $this->getQueryByKeyword($parts[$id], null, $url[0], $url[1]) : $this->getQueryByKeyword($parts[$id]);
		...
		foreach ($data as $key => $value) {
			...
			switch ($key) {
				...
				case 'path':
					...
					if ($this->config->get('config_seo_url_short_category_path')) {
	...
	private function getQueryByKeyword($keyword, $language_id = null, $route = null, $parent_id = null) {
		$query = null;
		...
		if ($this->config->get('config_seo_url_cache')){
			...
			if (isset($parent_id) && ($route == 'category_id' || $route == 'blog_category_id')) {
				switch ($route) {
					case 'category_id':
						$_query = $this->db->query("SELECT a.query FROM " . DB_PREFIX . "seo_url a, " . DB_PREFIX . "category c WHERE a.keyword = '" . $this->db->escape($keyword) . "' AND a.store_id = '" . $store_id . "' AND a.language_id = '" . $language_id . "' AND a.query=CONCAT('category_id=', c.category_id) AND c.parent_id='" . $parent_id . "'");
						break;
					case 'blog_category_id':
						$_query = $this->db->query("SELECT a.query FROM " . DB_PREFIX . "seo_url a, " . DB_PREFIX . "blog_category c WHERE a.keyword = '" . $this->db->escape($keyword) . "' AND a.store_id = '" . $store_id . "' AND a.language_id = '" . $language_id . "' AND a.query=CONCAT('blog_category_id=', c.blog_category_id) AND c.parent_id='" . $parent_id . "'"); break;
					default:
						break;
				}
				if (!$_query->rows) {
