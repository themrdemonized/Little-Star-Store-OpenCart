...
class ModelCatalogWarehouse extends Model {
	...
	public function getWarehouseByProduct($product_id, $option_value_id) {
		if (!$this->getWarehouses()) {
			return NULL;
		}

		$query = $this->db->query("SELECT w.warehouse_id, w.working_hours, wd.name, wd.address, wp.product_id, wp.option_value_id, wp.quantity FROM `" . DB_PREFIX ."warehouse` w LEFT JOIN `" . DB_PREFIX . "warehouse_description` wd ON (w.warehouse_id = wd.warehouse_id) AND (wd.language_id = " . (int)$this->config->get('config_language_id') . ") LEFT JOIN `" . DB_PREFIX . "warehouse_product` wp ON (w.warehouse_id = wp.warehouse_id) AND (wp.product_id = " . (int)$product_id . ") AND (wp.option_value_id = " . (int)$option_value_id . ")");
	...
	public function getQuantityByOptionValueId($product_id, $option_value_id) {

		if (!$option_value_id) {
			return NULL;
		}

		$query = $this->db->query("SELECT quantity FROM `" . DB_PREFIX ."product_option_value` WHERE product_id = " . (int)$product_id . " AND option_value_id = " . (int)$option_value_id);
		if (!empty($query->row)) {
