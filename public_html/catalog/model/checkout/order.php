...
//Connect to Trade and send order
	private function connect($order_data, $order_id) {
		$this->load->model('extension/module/trade_import');
    	$url = $this->config->get('module_trade_import_order_address');
    	$token = $this->config->get('module_trade_import_order_token');
    	$time = time();
    	$date = date('c', $time);
    	$data = array();
    	$data['orders'] = array();
    	$data['orders'][] = array(
    		'date' 			=> $date,
    		'description'	=> $order_data['store_url'] . " - " . $order_data['invoice_prefix'] . "-" . $order_id,
    	);
        ...
        if (json_decode($response) !== NULL) {
        	$this->model_extension_module_trade_import->add_order($data_string, $order_id, $response);
        } else {
        	$this->model_extension_module_trade_import->add_order($data_string, $order_id, '0');
	...
	public function getOrderProducts($order_id) {
		$query = $this->db->query("SELECT op.*, p.price AS price_old, ps.discount AS discount, pd.price AS price_discount, p.width, p.length, p.height, p.weight FROM " . DB_PREFIX . "order_product op LEFT JOIN " . DB_PREFIX . "product p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_special ps ON op.product_id = (SELECT MIN(psid.product_special_id) FROM " . DB_PREFIX . "product_special psid WHERE op.product_id = psid.product_id) LEFT JOIN " . DB_PREFIX . "product_discount pd ON (op.product_id = pd.product_id AND pd.customer_group_id = (SELECT oid.customer_group_id FROM " . DB_PREFIX . "order oid WHERE oid.order_id = op.order_id)) WHERE op.order_id = '" . (int)$order_id . "'");
		
		return $query->rows;
	}
	
	public function getOrderOptions($order_id, $order_product_id) {
		$query = $this->db->query("SELECT oo.*, pov.price_old AS price_old, pov.discount AS discount FROM " . DB_PREFIX . "order_option oo LEFT JOIN " . DB_PREFIX . "product_option_value pov ON (oo.product_option_value_id = pov.product_option_value_id) WHERE oo.order_id = '" . (int)$order_id . "' AND oo.order_product_id = '" . (int)$order_product_id . "'");
	...
	public function getOrderTotalShipping($order_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_total` WHERE order_id = '" . (int)$order_id . "' AND `code` = 'shipping' ORDER BY sort_order ASC");
