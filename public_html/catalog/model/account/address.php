...
class ModelAccountAddress extends Model {
	...
	public function deleteAddress($address_id) {
		...
		if ($address_id == $this->customer->getAddressId()) {
			$first_id = $this->db->query("SELECT address_id FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$this->customer->getId() . "' LIMIT 1");
			$first_id = isset($first_id->row['address_id']) ? (int)$first_id->row['address_id'] : 0;
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET address_id = '" . $first_id . "' WHERE customer_id = '" . (int)$this->customer->getId() . "'");
		}
	}
	...
	public function getAddresses() {
		...
		uasort($address_data, function($a, $b) use ($address_id) {
			if ($a['address_id'] != $address_id && $b['address_id'] != $address_id) {
				return 0;
			}
			return $a['address_id'] == $address_id ? -1 : 1;
		});
