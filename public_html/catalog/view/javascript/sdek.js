...
function chooseFirstPvz(tariff, tariff_type) {

    //console.log('i am shitty pvz');
    getPvzList(tariff_type);

    selected_tariff = tariff;

    pvzCodes = pvzlist.map(function(value, index) {
        return value['Code'];
    });

    //console.log(pvzCodes);
    var pvzIndex = Math.max(pvzCodes.indexOf(selectedPvzCode), 0);
    selectPvz(pvzlist[pvzIndex].Code);
}
    ...
    $.ajax ({
        ...
        success: function(json) {
            ...
            if(json.status) {
                ...
                $('#cdek_selectedPvzInfo_'+selected_tariff+', .cdek_selectedPvzInfo').html(json.data.address);
            }
        },
        error: function(data) {
