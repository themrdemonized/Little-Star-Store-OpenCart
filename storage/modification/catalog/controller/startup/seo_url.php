...
class ControllerStartupSeoUrl extends Controller {
		...
		//seopro prepare route end
			...
			foreach ($parts as $part) {
				...
				if ($query->num_rows) {
					...
					if ($query->row['query'] && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id' && $url[0] != 'stocks_id') {
			...
			if (!isset($this->request->get['route'])) {
				if (isset($this->request->get['product_id'])) {
					...
					$this->request->get['route'] = 'product/stocks/stocks';
				}
			}
		}
	...
	public function rewrite($link) {
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				if (($data['route'] == 'product/product' && $key == 'product_id') || (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product') && $key == 'manufacturer_id') || ($data['route'] == 'information/information' && $key == 'information_id') || ($data['route'] == 'product/stocks/stocks' && $key == 'stocks_id')) {
		...
		//seo_pro add blank url
			...
			// if (str_replace('/index.php', '', $url_info['path']) . $url . $query == '') {
			// 	$url = '/';
			// }
