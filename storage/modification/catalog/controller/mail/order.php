...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['logo'] = $order_info['store_url'] . 'image/catalog/logo.png';
		...
		$data['payment_address_fields'] = $replace;
		...
		$data['shipping_address_fields'] = $replace;
		...
		foreach ($order_products as $order_product) {
			...
			foreach ($order_options as $order_option) {
				...
				$option_price_old = $order_option['price_old'];
			}
			...
			$data['products'][] = array(
				...
				'price_old' => $this->currency->format(isset($option_price_old) ? $option_price_old : (isset($order_product['price_discount']) ? $order_product['price_discount']  : $order_product['price_old']) + ($this->config->get('config_tax') ? $order_product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
