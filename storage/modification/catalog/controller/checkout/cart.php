	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				$json['metrika'] = array();
				$json['metrika']['id'] = $product_info['model'] ?: $product_info['product_id'];
				$json['metrika']['name'] = $product_info['name'];
				if ((float)$product_info['special']) {
                    $json['metrika']['price'] = $this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax'));     
                } else {
                    $json['metrika']['price'] = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')); 
                }
                $categories = array();
				foreach (array_column($this->model_catalog_product->getCategories($product_info["product_id"]), 'category_id') as $category_id) {
					$categories[] = $this->model_catalog_category->getCategory($category_id)['name'];
				}
				$json['metrika']['category'] = implode("/", $categories);
				$json['metrika']['quantity'] = $quantity;
				$json['metrika']['variant'] = $option;
				...
				$json['sum'] = $this->currency->format($total, $this->session->data['currency']);
				$json['total'] = (string) $cart_count;
		...
		if (isset($this->request->post['key'])) {
            ...
            foreach ($this->cart->getProducts() as $cart_product) {
                if ($cart_product["cart_id"] == $this->request->post['key']) {
					...
					$json['metrika']['category'] = implode("/", $categories);
					$json['metrika']['quantity'] = $cart_product['quantity'];
					$json['metrika']['variant'] = $cart_product['option'];
